...
import {
  RMA_ASKING_SENTENCES,
  RMA_APPROVING_SENTENCES,
  RMA_TEMPLATE_BODY,
} from "../data/rma";
...
const QUICK_LINKS = [
  {
    label: "RMA Action Determinator",
    url: "https://app.powerbi.com/groups/me/reports/e730b13c-17a6-48fb-a890-0529d11238b7/ReportSection?experience=power-bi",
  },
  {
    label: "SN Lookup",
    url: "https://app.powerbi.com/groups/me/reports/9fe2da5c-962d-4b7f-bb98-ddaeebbcdf48/ReportSection296f430d36997650d4af?experience=power-bi",
  },
];

const CUSTOMER_ISSUE_CATEGORIES: Record<string, string[]> = {
  Booting: [
    'Blue screen', 'Booting', 'Cannot get into setup',
    'Locks up', 'Reboots', 'Stuck in bootloader', 'Stuck in lsys screen',
  ],
  Communication: [
    'Communication Cresnet', 'Communication Digital I/O', 'Communication Ethernet',
    'Communication IR', 'Communication RF', 'Communication Serially/RS-232', 'Communication USB',
  ],
  Power: [
    'No backlight', 'No display', 'No power at all',
    'Power light on but no display, no activity', 'Power surge/Lightning', 'Shuts down',
  ],
  'Touch screen': ['Cannot calibrate / loses calibration', 'Unresponsive'],
}
...
function buildFilledCopy(units: UnitRow[], fields: RmaFields): string {
  ...
  if (units.length === 1) {
    ...
    units.forEach((u, i) =>
      lines.push(
        `Unit ${i + 1} — Serial: ${u.serialNumber || "—"} | Product: ${u.productName || "—"}`,
      ),
    );
  }
  ...
  return (
    <div className="h-full flex flex-col gap-3 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0 gap-4">
        <div>
          <h1 className={`text-2xl font-bold tracking-tight mb-0.5 ${tk.tp}`}>
            RMA
          </h1>
          <p className={`text-sm ${tk.ts}`}>
            Return Merchandise Authorization — templates &amp; form
          </p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex gap-3">
        {/* ── LEFT: Fill-in form ─────────────────────────────── */}
        <div
          className={`flex-1 min-w-0 rounded-xl border flex flex-col overflow-hidden ${tk.panel}`}
        >
          <div className="px-4 pt-3 pb-2 flex-shrink-0 flex items-center justify-between">
            <p
              className={`text-[10px] font-semibold tracking-widest uppercase ${tk.tx}`}
            >
              RMA Details — Fill in
            </p>
            <button
              onClick={() =>
                doCopy("left-filled", buildFilledCopy(units, fields))
              }
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 flex flex-col gap-3">
                  ...
                  <div key={i} className="flex items-center gap-2">
                    <div className="flex-1 grid grid-cols-2 gap-2">
                      <div>
                        {i === 0 && (
                          <label
                            className={`block text-[10px] font-semibold mb-0.5 ${tk.tx}`}
                          >
                            Serial Number
                          </label>
                        )}
                        <input
                          ...
                          onChange={(e) =>
                            updateUnit(i, "serialNumber", e.target.value)
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateUnit(i, "productName", e.target.value)
                          }
            ...
            <div>
              <label
                className={`block text-[10px] font-semibold mb-1 ${tk.tx}`}
              >
                Issue Description
              </label>
              <textarea
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-3">
          {/* Asking RMA template */}
          <div
            className={`rounded-xl border flex-1 min-h-0 flex flex-col overflow-hidden ${tk.panel}`}
          >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-3 flex flex-col gap-2.5">
              ...
              {askingSentences.map((sentence, i) => {
                ...
                return (
                  <div
                    key={key}
                    className={`flex items-start gap-2.5 rounded-lg border p-2.5 ${tk.surface}`}
                  >
                    <span
                      className={`text-[10px] font-bold mt-0.5 w-14 flex-shrink-0 ${accentText}`}
                    >
                      Option {i + 1}
                    </span>
                    <span className={`text-xs flex-1 leading-relaxed ${tk.ts}`}>
                      {sentence}
                    </span>
                    <button
                      onClick={() => doCopy(key, copyText)}
                      className={copyBtnClass(key)}
                    >
              ...
              <div className={`rounded-lg border px-3 py-2.5 ${tk.surface}`}>
                <p
                  className={`text-[10px] font-semibold tracking-widest uppercase mb-2 ${tk.tx}`}
                >
                  Template preview
                </p>
                <div className="flex flex-col gap-1">
                  {[
                    ...
                    {
                      label: "Delivery/return address confirmation",
                      indent: false,
                      isHeader: true,
                    },
                    ...
                    <div
                      key={label}
                      className={`flex items-center gap-1.5 ${indent ? "pl-4" : ""}`}
                    >
                      {!isHeader && (
                        <span className={`text-xs ${templateLabel}`}>
                          {label}:
                        </span>
                      )}
                      {isHeader && (
                        <span className={`text-xs font-medium ${tk.ts}`}>
                          {label}
                        </span>
                      )}
                      {!isHeader && (
                        <span
                          className={`text-xs flex-1 border-b ${templateLine}`}
                          style={{ height: "1px", minWidth: "40px" }}
                        />
                      )}
                    </div>
                  ))}
              ...
              <div className={`rounded-lg border px-3 py-2.5 ${tk.surface}`}>
                <div className="flex items-center justify-between mb-2">
                  <p className={`text-[10px] font-semibold tracking-widest uppercase ${tk.tx}`}>
                    Customer Reported Issue
                  </p>
                  <button
                    onClick={() =>
                      doCopy('cri', `Customer Reported Issue 1: ${issueCategory} | Customer Reported Issue 2: ${issueSubCategory}`)
                    }
                    disabled={!issueCategory || !issueSubCategory}
                    className={`${copyBtnClass('cri')} disabled:opacity-30 disabled:cursor-not-allowed`}
                  >
                    {copiedKey === 'cri' ? <CheckIcon /> : <CopyIcon />}
                    {copiedKey === 'cri' ? 'Copied' : 'Copy'}
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <select
                    value={issueCategory}
                    onChange={(e) => { setIssueCategory(e.target.value); setIssueSubCategory(''); }}
                    className={`w-full px-2.5 py-1.5 rounded-lg border text-xs outline-none transition-all ${tk.inp}`}
                  >
                    <option value="">Issue Category</option>
                    {Object.keys(CUSTOMER_ISSUE_CATEGORIES).map((cat) => (
                  ...
                  <select
                    ...
                    className={`w-full px-2.5 py-1.5 rounded-lg border text-xs outline-none transition-all disabled:opacity-40 ${tk.inp}`}
                  >
                    <option value="">Sub-issue</option>
                    {(CUSTOMER_ISSUE_CATEGORIES[issueCategory] ?? []).map((sub) => (
                      <option key={sub} value={sub}>{sub}</option>
                    ))}
                  </select>
                </div>
                {issueCategory && issueSubCategory && (
                  <p className={`text-[10px] mt-1.5 ${tk.ts}`}>
                    Issue 1: <span className={tk.tp}>{issueCategory}</span>
                    {' · '}
                    Issue 2: <span className={tk.tp}>{issueSubCategory}</span>
                  </p>
                )}
              </div>
            </div>
          ...
          <div className={`rounded-xl border flex-shrink-0 ${tk.panel}`}>
            ...
            <div className="px-4 pb-3">
              <div
                className={`rounded-lg border p-3 flex items-start gap-3 ${tk.surface}`}
              >
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-medium mb-1 ${tk.tp}`}>
                    {approvingSentences[0]}
                  </p>
                  <p className={`text-xs ${tk.ts}`}>{approvingSentences[1]}</p>
                </div>
                <button
                  onClick={() =>
                    doCopy(`approve-${language}`, approvingSentences.join("\n"))
                  }
                  className={copyBtnClass(`approve-${language}`)}
                >
                  {copiedKey === `approve-${language}` ? (
                    <CheckIcon />
                  ) : (
                    <CopyIcon />
                  )}
