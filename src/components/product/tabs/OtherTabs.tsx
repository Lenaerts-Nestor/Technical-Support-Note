...
export function TechTipsTab({ product }: { product: Product }) {
  const { tk } = useTk()
  if (!product.techTips.length) return <EmptyState message="No tech tips yet for this product." />
  return (
    <div className="space-y-3">
      {product.techTips.map((item, i) => (
        <div key={i} className={`flex gap-3 rounded-xl border p-4 ${tk.panel}`}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#b45309" strokeWidth="2" className="flex-shrink-0 mt-0.5">
    ...
    <div className="space-y-4">
      {product.commonProblems.map((item, i) => (
        <div key={i} className={`rounded-xl border ${tk.panel}`}>
          <div className={`flex items-center gap-2.5 px-4 py-3 border-b ${tk.brd}`}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#c0392b" strokeWidth="2">
              <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
            </svg>
            <h3 className={`text-sm font-semibold ${tk.tp}`}>{item.problem}</h3>
          </div>
          <div className="px-4 py-3">
            <ol className="space-y-2.5">
              {item.solutions.map((sol, si) => (
                <li key={si} className="flex gap-2.5 items-start">
                  <span className={`flex-shrink-0 w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center mt-0.5 ${tk.stepBubble}`}>
                    {si + 1}
                  </span>
                  <div className="flex-1 flex items-start justify-between gap-2">
                    <p className={`text-sm leading-relaxed ${tk.ts}`}>{sol}</p>
                    <CopyButton text={sol} />
                  </div>
                </li>
              ))}
...
export function MyNotesTab({ notes, onNotes }: NotesProps) {
  ...
  return (
    <div className="space-y-3">
      <div className={`flex items-start gap-2 rounded-xl border p-3 text-xs ${tk.tip}`}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="flex-shrink-0 mt-0.5">
          <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
        </svg>
        <span>Notes are stored in-memory only and will reset on page refresh.</span>
      </div>
      <textarea
        value={notes}
...
export function LinksTab({ product }: { product: Product }) {
  ...
  if (!product.links.length) {
    return (
      <div className={`rounded-xl border p-8 text-center ${tk.panel}`}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={`mx-auto mb-2 ${tk.tx}`}>
          <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
          <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
        </svg>
        <p className={`text-sm ${tk.ts}`}>No links yet.</p>
        <p className={`text-xs mt-1 ${tk.tx}`}>Add documentation URLs in src/data/products.ts</p>
      </div>
    )
  }
  return (
      ...
      {product.links.map((link, i) => (
        ...
        >
          <div>
            ...
            {link.description && <p className={`text-xs mt-0.5 ${tk.ts}`}>{link.description}</p>}
          </div>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`flex-shrink-0 ${tk.tx}`}>
