...
export function ProductInfoTab({ product }: { product: Product }) {
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-xl border p-4 text-sm leading-relaxed ${tk.panel} ${tk.ts}`}>
        {product.description}
      </div>
      {product.specs.map((sec, si) => (
        <div key={si} className={`rounded-xl border ${tk.panel}`}>
          <div className={`flex items-center gap-2 px-4 py-3 border-b ${tk.brd}`}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={tk.accentColor} strokeWidth="2">
          ...
          <div className="px-4">
            {sec.fields.map((f, fi) => (
              <div key={fi} className={`flex justify-between items-center py-2.5 text-sm ${fi < sec.fields.length - 1 ? `border-b ${tk.row}` : ''}`}>
